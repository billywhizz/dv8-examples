function threadFunc () {
  ...
  module.exports = {
    start: () => {
      thread = process.spawn(threadFunc, ({ err, thread, status }) => {
        //print('thread finished')
        //print(`status: ${status}`)
        //if (err) print(`err:    ${err.message}`)
        thread.sock.close()
      }, { ipc: true })
      thread.onMessage(message => {
        //print(`process.receive: ${message.length}`)
        global.send(message.payload)
      })
      global.receive = message => {
        //print(`process.send: ${message.length}`)
        //print(message)
